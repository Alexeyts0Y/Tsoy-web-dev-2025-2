...
from .auth import check_rights # Импортируем декоратор check_rights
...
def show(user_id):
    ...
    return render_template('users/show.html', user_data=user)
    ...
    if request.method == 'POST':
        ...
        if not any(errors.values()):
            try:
                ...
                return redirect(url_for('users.index'))
            except connector.errors.IntegrityError as e: # Обработка ошибки уникальности логина
...
def delete(user_id):
    if user_id == current_user.id:
        flash('Вы не можете удалить свой собственный аккаунт через этот интерфейс.', 'danger')
        return redirect(url_for('users.index'))
...
def edit(user_id):
    ...
    errors = {k: v for k, v in errors.items() if v}
    return render_template('users/edit.html', user_data=user, roles=role_repository.all(), errors=errors, current_user=current_user)
